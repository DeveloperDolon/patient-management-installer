    ...
    <div class="w-full sm:px-6">
        <div class="rounded-tl-lg rounded-tr-lg bg-gray-100 px-4 py-4 md:px-10 md:py-7">
            <div class="items-center justify-center sm:flex">
                ...
                >
                    Special Procedure Management
                </p>
            </div>

            <div class="mt-5 grid grid-cols-1 gap-5 md:mt-10 md:grid-cols-2 md:gap-20">
                <form method="POST" class="flex gap-5" action="{{ route('dashboard.special-procedures.store') }}">
                    @csrf
                ...
                <x-search
                    :data="[
                        'route' => 'dashboard.special-procedures',
                        'search' => $search,
                    ]"
                />
            </div>
        ...
        <x-pagination :data="[
            'links' => $specialProcedures->toArray()['links'],
        ]" />
    </div>
@endsection
